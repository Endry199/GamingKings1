...
export async function handler(event) {
    ...
    try {
        ...
        const catalogResponse = await fetch(`${API_BASE}/products/catalog`, { headers: { Authorization: `Bearer ${process.env.RECARGAS_AMERICA_API_TOKEN}`, Accept: 'application/json' } });
        ...
        const validationResponse = await fetch(`${API_BASE}/catalog/validate`, {
            method: 'POST',
            headers: { Authorization: `Bearer ${process.env.RECARGAS_AMERICA_API_TOKEN}`, 'Content-Type': 'application/json', Accept: 'application/json' },
            body: JSON.stringify({ product_id: product.id, service_user_id: String(serviceUserId).trim() })
        });
        ...
        // 🆕 Si el proveedor no soporta precheck (supported: false), devolvemos "válido sin nombre" para no bloquear
        if (validation.data?.supported === false) {
            return json(200, { ok: true, valid: true, accountName: null, productId: product.id, productName: product.name, noPrecheck: true });
        }
