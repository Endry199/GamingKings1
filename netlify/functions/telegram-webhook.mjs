...
export async function handler(event) {
  ...
  try {
    ...
    const manualOrder = action === 'manual_complete' || action === 'manual_reject';
    const approved = action === 'approve' || action === 'manual_complete';
    ...
    const status = approved ? (manualOrder ? 'completado' : 'aprobado') : 'rechazado';
    ...
    if (!manualOrder && approved && transaction.google_id) {
    ...
    if (!approved && manualOrder && transaction.google_id) {
      const amount = Number(transaction.base_amount || transaction.finalPrice || 0);
      const { data: balance } = await supabaseAdmin.from('saldos').select('saldo_ncoins').eq('user_id', transaction.google_id).maybeSingle();
      const current = Number(balance?.saldo_ncoins || 0);
      await supabaseAdmin.from('saldos').upsert({ user_id: transaction.google_id, saldo_ncoins: current + amount });
    }
    if (transaction.email) {
      stage = 'status_email';
      const subject = manualOrder ? (approved ? 'Tu pedido fue completado · Niunx Play' : 'Tu pedido fue rechazado · Niunx Play') : (approved ? 'Tu recarga ya está disponible · Niunx Play' : 'Actualización de tu recarga · Niunx Play');
      const text = manualOrder ? (approved ? 'Tu pedido fue completado correctamente.' : 'Tu pedido fue rechazado y el saldo fue reintegrado a tu wallet.') : (approved ? 'Tu recarga fue aprobada.' : 'Tu recarga fue rechazada.');
      await mailer().sendMail({ from: process.env.SMTP_FROM || process.env.SMTP_USER, to: transaction.email, subject, text, html: emailHtml(approved, transaction, manualOrder) });
    }
    await answerTelegram(callback.id, manualOrder ? (approved ? 'Pedido marcado como completado.' : 'Pedido rechazado y saldo reintegrado.') : (approved ? 'Pago aprobado y saldo actualizado.' : 'Pago rechazado.'));
