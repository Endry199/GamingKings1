...
exports.handler = async function(event, context) {
    ...
    // ⭐️ MODIFICADO: Se removió 'email' de la desestructuración. Se extraen otros campos necesarios.
    const { finalPrice, currency, paymentMethod, whatsappNumber, cartDetails } = data; 

    // 🎯 FIX CLAVE: Obtener el ID de Google. Se prueba con 'google_id', 'user_id', y 'userId'
    const google_id = data.google_id || data.user_id || data.userId;

    // 🛑 NUEVA VALIDACIÓN: Se requiere el google_id (ahora extraído de forma más robusta)
    if (!google_id) { 
         return {
            statusCode: 400,
            body: JSON.stringify({ message: "Falta el ID de Google (google_id) para buscar el email. Intente iniciar sesión de nuevo." })
        };
    }

    // 🚀 NUEVA LÓGICA: Obtener el email del usuario de Supabase
    let email = null; // Variable para almacenar el email de la DB
    let user_id_db = null; // Variable para almacenar el ID interno (bigint)

    try {
        const { data: userData, error: userError } = await supabase
            .from('usuarios') // ⭐️ TABLA CORRECTA: 'usuarios'
            .select('email, id') // Obtener 'email' y el 'id' interno
            .eq('google_id', google_id) // ⭐️ COLUMNA CORRECTA: Buscar por 'google_id'
            .single();
        ...
        if (userData && userData.email) {
            ...
            user_id_db = userData.id; // Guarda el ID interno (bigint)
            console.log(`Email obtenido de Supabase: ${email} (ID interno: ${user_id_db})`);
        } else {
            console.error(`Usuario no encontrado en Supabase con Google ID: ${google_id}`);
            return {
                statusCode: 404,
                body: JSON.stringify({ message: `Usuario no encontrado o sin email registrado para ID: ${google_id}` })
            };
        }
    } catch (dbError) {
        console.error("Error al buscar el email en Supabase:", dbError.message);
        return {
            statusCode: 500,
            body: JSON.stringify({ message: "Error al buscar el email en la base de datos." })
        };
    }
    // 🚀 FIN NUEVA LÓGICA
    ...
    try {
        ...
        const transactionToInsert = {
            ...
            email: email, // ⭐️ MODIFICADO: Usa el email obtenido de la DB
            ...
            // Campo para el Google ID de la billetera (del item) o del usuario (request)
            google_id: firstItem.google_id || google_id || null, // ⭐️ MODIFICADO: Guarda el google_id del usuario también
            user_id: user_id_db || null, // ⭐️ AÑADIDO: Guarda el ID interno (bigint)
    ...
    messageText += `📧 Correo Cliente: ${email}\n`; // ⭐️ MODIFICADO: Usa el 'email' obtenido de la DB
    messageText += `🆔 Google ID Cliente: ${google_id}\n`; // ⭐️ AÑADIDO: Muestra el ID usado para la búsqueda
    ...
    if (email) { // El 'email' ya está populado con el valor de la DB
        let transporter;
        try {
