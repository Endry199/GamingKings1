...
export async function handler(event) {
  ...
  try {
    ...
    await supabaseAdmin.from('email_verification_codes').insert({ email: email.toLowerCase(), code_hash: hash(verificationCode), purpose: 'register', expires_at: new Date(Date.now() + 10 * 60 * 1000).toISOString() });
