...
exports.handler = async function(event, context) {
    ...
    if (referenceNumber) {
        messageText += `📊 Referencia Pago: ${escapeMarkdownV2(referenceNumber)}\n`;
    }
    if (txid) {
    ...
    try {
        ...
        console.log(`SUCCESS: Mensaje de Telegram enviado para la transacción ${newTransactionData.id_transaccion}`);
        ...
        if (receiptUrl) {
            try {
                ...
                await axios.post(sendFileUrl, {
                    chat_id: TELEGRAM_CHAT_ID,
                    document: receiptUrl,
                    caption: escapeMarkdownV2(`Comprobante de pago para la transacción ${newTransactionData.id_transaccion}`),
                    parse_mode: 'MarkdownV2'
                });
                console.log("Comprobante de pago enviado a Telegram usando URL de Supabase.");
                console.log(`SUCCESS: Comprobante de pago enviado para la transacción ${newTransactionData.id_transaccion}`);
            } catch (fileSendError) {
        ...
        if (newTransactionData && telegramMessageResponse && telegramMessageResponse.data && telegramMessageResponse.data.result) {
            ...
            if (updateError) {
                ...
                console.log(`SUCCESS: Supabase actualizado con message_id para la transacción ${newTransactionData.id_transaccion}`);
            }
        }
