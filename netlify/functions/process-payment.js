...
exports.handler = async function(event, context) {
    ...
    // Normalización de la moneda para la visualización en Telegram
    const normalizedCurrencyGlobal = currency ? currency.toUpperCase() : 'USD';
    let displayCurrency = normalizedCurrencyGlobal;
    if (normalizedCurrencyGlobal === 'USDM' || normalizedCurrencyGlobal === 'USDE') {
        // Si es la moneda premium, la llamamos USDE
        displayCurrency = 'USDE';
    } else if (normalizedCurrencyGlobal === 'USD') {
        displayCurrency = 'USD';
    }
    ...
    cartItems.forEach((item, index) => {
        ...
        let itemPrice;
        let itemCurrencyDisplay = displayCurrency; // Usamos la moneda normalizada global
        
        // Usamos la moneda de la transacción global para seleccionar el precio
        const itemCurrencyCheck = currency ? currency.toUpperCase() : 'USD'; 

        if (itemCurrencyCheck === 'USDM' || itemCurrencyCheck === 'USDE') { 
            // 🚀 CORRECCIÓN: Usamos priceUSDM (el precio de 0.97) para USDE/USDM
            itemPrice = item.priceUSDM;
            itemCurrencyDisplay = 'USDE';
            console.log(`[DEBUG] LÓGICA APLICADA: GLOBAL USDE/USDM. Price usado: ${itemPrice}. Fuente: item.priceUSDM`);
        } else if (itemCurrencyCheck === 'VES') {
            // Lógica VES
            itemPrice = item.priceVES;
            itemCurrencyDisplay = 'VES';
            console.log(`[DEBUG] LÓGICA APLICADA: GLOBAL VES. Price usado: ${itemPrice}. Fuente: item.priceVES`);
        } else {
            // Lógica USD (o fallback, el precio de 0.88)
            itemPrice = item.priceUSD;
            itemCurrencyDisplay = 'USD';
        ...
        if (itemPrice) {
            // Usamos el itemCurrencyDisplay ajustado
            messageText += `💲 Precio (Est.): ${parseFloat(itemPrice).toFixed(2)} ${itemCurrencyDisplay}\n`;
        }
    ...
    messageText += `\n*RESUMEN DE PAGO*\n`;
    // 🚀 CORRECCIÓN: Usamos la moneda ajustada (displayCurrency) para el total
    messageText += `💰 *TOTAL A PAGAR:* *${finalPrice} ${displayCurrency}*\n`; 
        ...
        const mailOptions = {
            ...
            html: `
                <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
                    ...
                    <p><strong>Monto Total a Pagar:</strong> <span style="font-size: 1.1em; color: #d9534f; font-weight: bold;">${finalPrice} ${displayCurrency}</span></p>
